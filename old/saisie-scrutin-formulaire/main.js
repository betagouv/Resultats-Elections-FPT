...
const generateForm = () => {
  ...
  const inscritInput = generateInputText(inscrits[0], getHtmlType(inscrits[0].type))
  ...
  for (let i = 0; i < resultatsMapped.length; i++){
    ...
    const input = isTypeText ? generateInputText(coloneInfo, type, 'fr-col-12')
  ...
  for (let i = 0; i < syndicats.length; i++){
    ...
    const isTypeText = type === 'number' || type === 'text'
    const input = isTypeText ? generateInputText(syndicats[i], type, 'fr-col-6')
...
const checkAbsence = () => {
  inputsToUpdate = [...votesInputs.querySelectorAll('input'), ...syndicatInputs.querySelectorAll('input')]
